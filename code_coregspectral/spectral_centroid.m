function [U U1 F P R nmi avgent AR] = spectral_centroid(X1,X2,numClust,sigma1,sigma2,lambda1,lambda2,truth,numiter)
% INPUT:
% OUTPUT:
    ...
    F(2) = mean(Fj); std_F(2) = std(Fj);
    P(2) = mean(Pj); std_P(2) = std(Pj);
    R(2) = mean(Rj); std_R(2) = std(Rj);
    nmi(2) = mean(nmi_j); std_nmi(2) = std(nmi_j);
    avgent(2) = mean(avgent_j); std_avgent(2) = std(avgent_j);
    AR(2) = mean(ARj); std_AR(2) = std(ARj);
    ...
    while(i<=numiter+2)
        ...
        L_ustar = (L_ustar+L_ustar')/2;
        [U1 E1] = eigs(L1 + lambda1*L_ustar, numEV,'LA',opts);    
        objval_u1(i) = sum(diag(E1));
        [U2 E2] = eigs(L2 + lambda2*L_ustar, numEV,'LA',opts);    
        objval_u2(i) = sum(diag(E2));
