...
function [x_out, e_out, pditer, timeSteps, errorSteps, idSteps] = SolvePDIPA_CBM(A, b, varargin)
...
while (pditer < pdmaxiter)
    ...
    errorSteps(pditer) = norm(xp(1:N)-xG) ;
    sci = zeros(1,recData.nSubj);
    norm_xk = norm(xp(1:N),1);
    for i = 1:recData.nSubj
        sci(i) = (recData.nSubj * norm(xp(((i-1)*recData.nImg+1):i*recData.nImg), 1)/norm_xk - 1) ...
            / (recData.nSubj - 1);
    end
    [dontCare, curId] = max(sci);
    idSteps(pditer) = (curId == recData.id);
