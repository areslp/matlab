...
%
%   [RMS]
%      options.conformal = 1 if we want to use C-isomap variant
%
% Output: 
...
conformal = options.conformal;  % [RMS]
...
elseif ((mode == 1) & (use_dijk == 1))
     if n_fcn == 'k'
         ...
         for i=1:N
             D(i,ind((2+K):end,i)) = 0; 
             mean_dists(i) = mean(nonzeros(D(i,:)));
             if ((verbose == 1) & (rem(i,50) == 0)) 
                 disp([' Iteration: ' num2str(i) '     Estimated time to completion: ' num2str((N-i)*toc/60/50) ' minutes']); tic; 
             end
         end
...
elseif conformal
    disp([' [RMS] using C-Isomap variant']); 
    Mij = sqrt(mean_dists * mean_dists');
    Dconf = D ./ Mij;
    D = dijkstra(Dconf, landmarks);
else
...
for di = 1:length(dims)
     ...
     r2 = 1-corrcoef(reshape(real(Isomap_L2_distance(Y.coords{di}, Y.coords{di}(:,land_ind))),N*nl,1),D).^2; 
     R(di) = r2(2,1); 
     if (verbose == 1)
