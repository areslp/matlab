...
//  - implementation of dijkstra's algorithm based using Boyer's Fibonnaci heap
//  - matlab/mex interface
//***************************************************************************

//***************************************************************************
// dijkstra_maxdist.cpp
// Copyright (c) Ryan Schmidt, University of Toronto, 3/12/09
//  - added support to halt at maximum distance
//  - added tracking and return of upwind-neighbour indices (to allow
//     caller to compute shortest-paths information)
//  - changed a few int* to mwIndex* for matlab r2008a 64-bit build
//***************************************************************************
...
// [RMS] modified to halt when distance to nearest node is > dMaxDist   
void dodijk_sparse_maxdist( 
             double dMaxDist,       // [RMS]
             long int M,
             long int N,
             long int S,
             double   *D,
             double   *sr,
             mwIndex      *irs,     // [RMS] change from int* for r2008a
             mwIndex      *jcs,     // [RMS] change from int* for r2008a
             HeapNode *A,
             FibHeap  *theHeap,
             double *Nearest )
{
   int      finished;
   long int i,startind,endind,whichneighbor,ndone,index,closest;
   ...
   while ((finished==0) && (ndone < M))
   {
      ...
      // [RMS] halt if max distance reached
      if ( closestD > dMaxDist ) {
          //mexPrintf("early-out at %f > %f\n", closestD, dMaxDist);
          finished = 1;
          continue;
      }
...
{
  double    *sr,*D,*P,*SS,*Dsmall,*Psmall, dMaxDist;
  double    *Nearest;
  mwIndex      *irs,*jcs;   // [RMS] changed from int for matlab r2008a
  ...
  if (nrhs != 3) {
      ...
      mexErrMsgTxt( "Only 1 or 2 output argument allowed." );
  }
   ...
   // [RMS]
   if ( nlhs > 1 ) {
       //plhs[1] = mxCreateNumericMatrix(M, 1, mxINT32_CLASS, 0 );
       plhs[1] = mxCreateDoubleMatrix(M, 1, mxREAL );
       Nearest = mxGetPr(plhs[1]);
   } else
       Nearest = NULL;
   ...
   if (mxIsSparse( prhs[ 0 ] ) == 1)
   {
     ...
     for (i=0; i<MS; i++)
     {
        ...
        //mexPrintf("maxDist is %f\n", dMaxDist);
        dodijk_sparse_maxdist( dMaxDist, M,N,S,Dsmall,sr,irs,jcs,A,theHeap, Nearest );
