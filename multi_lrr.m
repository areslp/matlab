...
while ~convergenced
    ...
    J2b=J2;
    J3b=J3;
    [J1,svp1]=singular_value_shrinkage(Z1+W1/mu,1/mu);
    [J2,svp2]=singular_value_shrinkage(Z2+W2/mu,1/mu);
    [J3,svp3]=singular_value_shrinkage(Z3+W3/mu,1/mu);
    % update S_i
    S1b=S1;
    S2b=S2;
    S3b=S3;
    ...
    E2b=E2;
    E3b=E3;
    E1=l21(X1-X1*S1+Y1/mu,lambda/(2*mu));  % bug fixed, parameter should be lambda/(2*mu), not lambda/mu
    E2=l21(X2-X2*S2+Y2/mu,lambda/(2*mu)); 
    E3=l21(X3-X3*S3+Y3/mu,lambda/(2*mu)); 
    ...
    if mod(iter,50)==0
        ...
        fprintf(1,'svp1 %d,svp2 %d,svp3 %d\n',svp1,svp2,svp3);
    end
    ...
    if changeX <= epsilon && gap <=epsilon2 && changeZJ <= epsilon && changeZS <= epsilon
        ...
        fprintf(2,'svp1 %d,svp2 %d,svp3 %d\n',svp1,svp2,svp3);
    end
